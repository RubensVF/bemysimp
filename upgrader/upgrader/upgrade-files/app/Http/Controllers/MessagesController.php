...
//        DB::enableQueryLog(); // Enable query log
        // auth user unlocks
        $authUnlocks = Unlock::where('tipper_id', auth()->id())->get()->pluck('id')->toArray();
    ...
    public function attachMedia($message, Request  $r)
    {
       ...
       // validate message id
        ...
        $allowedMimes = ['image/jpg', 'image/jpeg', 'image/png', 'image/gif', 
                         ...
                         'video/mp4', 'video/webm', 'video/mov', 'video/ogg', 'video/qt', 
        ...
        if ($r->boolean('is_last')) {
            ...
            try {
                ...
                // Move this thing
                $fileName = Storage::disk(env('DEFAULT_STORAGE'))->putFile($fileDestination, $localFile);
